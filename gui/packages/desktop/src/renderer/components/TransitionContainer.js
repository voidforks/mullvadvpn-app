...
export default class TransitionContainer extends Component /*:: <TransitionContainerProps, State>*/ {
  ...
  pushTransition(nextProps /*: TransitionContainerProps*/) {
    ...
    this.setState({
      ...
      const compositeAnimation = Animated.parallel([Animated.timing(currentTranslationValue, {
        toValue: 0,
        easing: Animated.Easing.InOut(),
        duration: nextProps.duration
      }), Animated.timing(previousTranslationValue, {
  ...
  render() {
    ...
    return <View style={getStyles().transitionContainerStyle}>
        {previousChildren && <Animated.View key={previousChildren && previousChildren.key} style={[getStyles().animationDefaultStyle, previousChildrenAnimation]}>
            {previousChildren}
          </Animated.View>}
