...
export default class SvgMap extends React.Component /*:: <SvgMapProps, SvgMapState>*/ {
  ...
  shouldComponentUpdate(nextProps /*: SvgMapProps*/, nextState /*: SvgMapState*/) {
    return this.props.width !== nextProps.width || this.props.height !== nextProps.height || this.props.center[0] !== nextProps.center[0] || this.props.center[1] !== nextProps.center[1] || this.props.offset[0] !== nextProps.offset[0] || this.props.offset[1] !== nextProps.offset[1] || this.props.zoomLevel !== nextProps.zoomLevel || this.props.showMarker !== nextProps.showMarker || this.props.markerImagePath !== nextProps.markerImagePath || this.state.zoomCenter !== nextState.zoomCenter || this.state.zoomLevel !== nextState.zoomLevel;
  }
  ...
  render() {
    ...
    const userMarker = this.props.showMarker && <Marker key={`user-location-${this.props.center.join('-')}`} marker={{ coordinates: this.props.center }} style={markerStyle}>
        <image x="-30" y="-30" href={this.props.markerImagePath} />
      </Marker>;

    const countryMarkers = this.state.visibleCountries.map(item => <Marker key={`country-${item.id}`} marker={{ coordinates: item.geometry.coordinates }} style={markerStyle}>
        <text fill="rgba(255,255,255,.6)" fontSize="22" textAnchor="middle">
          {item.properties.name}
        </text>
      </Marker>);

    const cityMarkers = this.state.visibleCities.map(item => <Marker key={`city-${item.id}`} marker={{ coordinates: item.geometry.coordinates }} style={markerStyle}>
    ...
    return <ComposableMap width={this.props.width} height={this.props.height} style={mapStyle} projection={this._getProjection} projectionConfig={this._projectionConfig}>
        <ZoomableGroup center={this.state.zoomCenter} zoom={this.state.zoomLevel} disablePanning={false} style={zoomableGroupStyle}>
          <Geographies geography={geographyData} disableOptimization={true}>
            {(geographies, projection) => {
            return this.state.visibleGeometry.map(({ id }) => <Geography key={id} geography={geographies[id]} projection={projection} style={geographyStyle} />);
          }}
          </Geographies>
          <Geographies geography={statesProvincesLinesData} disableOptimization={true}>
            {(geographies, projection) => {
            return this.state.visibleStatesProvincesLines.map(({ id }) => <Geography key={id} geography={geographies[id]} projection={projection} style={stateProvinceLineStyle} />);
          }}
          </Geographies>
  ...
  _getProjection(width /*: number*/, height /*: number*/, config /*: {
    ...
    return geoTimes().scale(scale).translate([xOffset + width / 2, yOffset + height / 2]).rotate(rotation).precision(precision);
  }

  _getZoomCenter(center /*: [number, number]*/, offset /*: [number, number]*/, projection /*: Function*/, zoom /*: number*/) {
    const pos = projection(center);
    return projection.invert([pos[0] + offset[0] / zoom, pos[1] + offset[1] / zoom]);
  }

  _getViewportGeoBoundingBox(centerCoordinate /*: [number, number]*/, width /*: number*/, height /*: number*/, projection /*: Function*/, zoom /*: number*/) {
    ...
    return [Math.min(northWest[0], southEast[0]), Math.min(northWest[1], southEast[1]), Math.max(northWest[0], southEast[0]), Math.max(northWest[1], southEast[1])];
  }

  _shouldInvalidateState(nextProps /*: SvgMapProps*/) {
    const oldProps = this.props;
    return oldProps.width !== nextProps.width || oldProps.height !== nextProps.height || oldProps.center[0] !== nextProps.center[0] || oldProps.center[1] !== nextProps.center[1] || oldProps.offset[0] !== nextProps.offset[0] || oldProps.offset[1] !== nextProps.offset[1] || oldProps.zoomLevel !== nextProps.zoomLevel;
  }
